...
export class DoctoresEditarComponent implements OnInit {
  ...
  start():void{ 
    ...
    this.listService.getAll().subscribe(data=>{this.l$ =data});
    this.dialog.open(ModalDialogComponent, {data:{ _i:this.i, _listP: this.l$, _listD:this.l$, _type:0 }});
  ...
  ngOnInit(){
    ...
    if(_id!=0){
      this.service.Editar(_id).subscribe((data:Doctores)=>{ this.d = data; console.log(this.d);});
    }
    else {
      this.d = this.initType();
    }
